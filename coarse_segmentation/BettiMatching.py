...
class CubicalPersistence:
    ...
    def BarCode(self, color='r', ratio=1):
        ...
        if self.filtration == 'sublevel':
            max_val = max(intervals, key=lambda x:x[1] if (x[1]!=np.inf) else -np.inf)[1]
            min_val = min(intervals, key=lambda x:x[0])[0]
        else:
            max_val = max(intervals, key=lambda x:x[0])[0]
            min_val = min(intervals, key=lambda x:x[1] if (x[1]!=-np.inf) else np.inf)[1]
...
class InducedMatching:
    ...
    def BarCode(self, plot_image=False, colors=['r','g','grey'], ratio=1):
        ...
        if self.IP.CP_0.filtration == 'sublevel':
            max_val_0 = max(intervals_0, key=lambda x:x[1] if (x[1]!=np.inf) else -np.inf)[1]
            min_val_0 = min(intervals_0, key=lambda x:x[0])[0]
        else:
            max_val_0 = max(intervals_0, key=lambda x:x[0])[0]
            min_val_0 = min(intervals_0, key=lambda x:x[1] if (x[1]!=-np.inf) else np.inf)[1]
        if self.IP.CP_1.filtration == 'sublevel':
            max_val_1 = max(intervals_1, key=lambda x:x[1] if (x[1]!=np.inf) else -np.inf)[1]
            min_val_1 = min(intervals_1, key=lambda x:x[0])[0]
        else:
            max_val_1 = max(intervals_1, key=lambda x:x[0])[0]
            min_val_1 = min(intervals_1, key=lambda x:x[1] if (x[1]!=-np.inf) else np.inf)[1]
...
class ComposedWassersteinMatching:
    def __init__(self, likelihood, ground_truth, relative=False, reduced=False, filtration='sublevel', construction='V', comparison='union', valid='positive', training=False):
        ...
        self.unmatched_gt = self.CP_gt.get_intervals(refined=True)
        if not self.reduced: 
            self.matched = [[((0,np.inf),(0,np.inf),(0,np.inf))],[]]
            self.unmatched_lh[0].remove((0,np.inf))
            self.unmatched_comp[0].remove((0,np.inf))
            self.unmatched_gt[0].remove((0,np.inf))
        else:
            self.matched = [[],[]]
        self.match()
    ...
    def match(self):
        ...
        if not self.reduced:
            intervals_lh[0].remove(self.CP_lh.fine_to_coarse((0,np.inf)))
            intervals_comp[0].remove(self.CP_comp.fine_to_coarse((0,np.inf)))
            intervals_gt[0].remove(self.CP_gt.fine_to_coarse((0,np.inf)))
    ...
    def BarCode(self, plot_comparison=False, colors=['r','b','g'], ratio=1):
        ...
        if self.filtration == 'sublevel':
            max_val_lh = max(intervals_lh, key=lambda x:x[1] if (x[1]!=np.inf) else -np.inf)[1]
            min_val_lh = min(intervals_lh, key=lambda x:x[0])[0]
        else:
            max_val_lh = max(intervals_lh, key=lambda x:x[0])[0]
            min_val_lh = min(intervals_lh, key=lambda x:x[1] if (x[1]!=-np.inf) else np.inf)[1]
        if self.filtration == 'sublevel':
            max_val_gt = max(intervals_gt, key=lambda x:x[1] if (x[1]!=np.inf) else -np.inf)[1]
            min_val_gt = min(intervals_gt, key=lambda x:x[0])[0]
        else:
            max_val_gt = max(intervals_gt, key=lambda x:x[0])[0]
            min_val_gt = min(intervals_gt, key=lambda x:x[1] if (x[1]!=-np.inf) else np.inf)[1]
