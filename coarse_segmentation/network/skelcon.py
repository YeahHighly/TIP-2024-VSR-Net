...
class LUNet(nn.Module):
	__name__ = 'lunet'
	use_render = False
	def __init__(self, n_channels=1, n_classes=1, layers=(32,32,32,32,32), num_emb=128):
...
if __name__ == "__main__":
    #for debug   
    img = torch.rand(2, 3, 256, 256).cuda()#.to(torch.device('cuda:%d'%4))
    unet = LUNet(n_channels=3, n_classes=1).cuda()
    out = unet(img)
    print(out.size())