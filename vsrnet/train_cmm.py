...
import module
import dataloader
from loss import CMMLoss, CMMLossBase

# ------------------------ Argument Parser for Hyperparameters ------------------------
parser = argparse.ArgumentParser(description="cmm_segmentation")
parser.add_argument("--module", type=str, default='cmm', help="cmm segmentation backbone.")
parser.add_argument("--dataset", type=str, default='drive', help="refine segmentation dataset.")

parser.add_argument("--epochs", type=int, default=1000, help="Number of training epochs.")
parser.add_argument("--batch_size", type=int, default=12, help="Batch size for training and testing.")
...
def train():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for images, mappings, clusters in train_loader:
            # print(images.shape, mappings.shape, clusters.shape)
            images, mappings, clusters = images.to(DEVICE), mappings.to(DEVICE), clusters.to(DEVICE)

            optimizer.zero_grad()
            outputs = model(images, mappings)
            loss = criterion(outputs, clusters)
            loss.backward()
            optimizer.step()
        ...
        if epoch % args.eval_interval == 0 or epoch == args.epochs:
            ...
            model.eval()
            with torch.no_grad():
                for images, mappings, clusters in test_loader:
                    images, mappings = images.to(DEVICE), mappings.to(DEVICE)
                    outputs = model(images, mappings)
                    preds = outputs.cpu().numpy()
                    targets = clusters.cpu().numpy()
