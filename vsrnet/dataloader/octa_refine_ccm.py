...
class OCTA_CCM_Dataset(Dataset):
    def __init__(self, graph_dir, train=True):
        ...
        self.train = train
        if self.train:
            self.graph_files = self.graph_files[:int(len(self.graph_files)*0.8)]
        else:
            self.graph_files = self.graph_files[int(len(self.graph_files)*0.8):]
...
def get_octa_ccm_dataloader(graph_dir, train, batch_size=4, shuffle=True):
    """Returns DataLoader for preprocessed graph data."""
    dataset = OCTA_CCM_Dataset(graph_dir=graph_dir)
    return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_fn)


if __name__ == "__main__":
    graph_dir = "/home/hly/iMED-Code/TIP-2024-VSR-Net/dataset/OCTA-500/graph_data/"
    train_loader = get_octa_ccm_dataloader(graph_dir, train=True, batch_size=2, shuffle=True)
