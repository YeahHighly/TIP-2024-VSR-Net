...
class DRIVE_CMM_Dataset(Dataset):
    def __init__(self, root_dir, train=True):
        self.root_dir = root_dir
        self.train = train
        self.patch_size = 256

        self.rehabilitate_dir = os.path.join(root_dir, "training/rehabilitate/" if train else "testing/rehabilitate/")
...
def get_drive_cmm_dataloader(root_dir, batch_size=4, train=True, shuffle=True, num_workers=2):
    dataset = DRIVE_CMM_Dataset(root_dir=root_dir, train=train)
...
def test_cmm_dataset():
    ...
    train_loader = get_drive_cmm_dataloader(root_dir, batch_size=4, train=True)
